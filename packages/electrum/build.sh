MCHS_PKG_HOMEPAGE=https://electrum.org
MCHS_PKG_DESCRIPTION="Electrum is a lightweight Bitcoin wallet."
MCHS_PKG_LICENSE="MIT"
MCHS_PKG_MAINTAINER="@mchs"
MCHS_PKG_VERSION=4.0.5
MCHS_PKG_REVISION=4
MCHS_PKG_SRCURL=https://download.electrum.org/$MCHS_PKG_VERSION/Electrum-$MCHS_PKG_VERSION.tar.gz
MCHS_PKG_SHA256=6790407e21366186d928c8e653e3ab38476ca86e4797aa4db94dcca2384db41a
MCHS_PKG_DEPENDS="python, libsecp256k1"
MCHS_PKG_BUILD_IN_SRC=true
MCHS_PKG_PLATFORM_INDEPENDENT=true
# asciinema previously contained some files that python packages have in common
MCHS_PKG_CONFLICTS="asciinema (<< 1.4.0-1)"
...
mchs_step_make_install () {
	# Force reinstall:
	rm -rf $MCHS_PREFIX/lib/python${_PYTHON_VERSION}/site-packages/electrum
	rm -rf $MCHS_PREFIX/lib/python${_PYTHON_VERSION}/site-packages/Electrum-${MCHS_PKG_VERSION}-py${_PYTHON_VERSION}.egg*
	export PYTHONPATH=$MCHS_PREFIX/lib/python${_PYTHON_VERSION}/site-packages/
	YARL_NO_EXTENSIONS=1 MULTIDICT_NO_EXTENSIONS=1 \
		CFLAGS=-I$MCHS_PREFIX/include/python${_PYTHON_VERSION} \
		python${_PYTHON_VERSION} setup.py install --prefix=$MCHS_PREFIX
}
