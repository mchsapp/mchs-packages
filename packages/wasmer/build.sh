MCHS_PKG_HOMEPAGE=https://github.com/wasmerio/wasmer
MCHS_PKG_DESCRIPTION="A fast and secure WebAssembly runtime"
MCHS_PKG_LICENSE="MIT"
MCHS_PKG_MAINTAINER="@mchs"
MCHS_PKG_VERSION=2.0.0
MCHS_PKG_SRCURL=https://github.com/wasmerio/wasmer/archive/${MCHS_PKG_VERSION}.tar.gz
MCHS_PKG_SHA256=f0d86dcd98882a7459f10e58671acf233b7d00f50dffe32f5770ab3bf850a9a6
...
mchs_step_pre_configure() {
	export WASMER_INSTALL_PREFIX="$MCHS_PREFIX"
...
mchs_step_make() {
	...
	# make build-wapm
	#wapm_version="$(grep "WAPM_VERSION = " "$MCHS_PKG_SRCDIR/Makefile" | sed -e "s/.* = //")"
...
mchs_step_make_install() {
	# make install-wasmer
	install -Dm755 -t "$MCHS_PREFIX/bin" "target/$CARGO_TARGET_NAME/release/wasmer"

	# make install-capi-headers
	for header in lib/c-api/*.h; do
		install -Dm644 "$header" "$MCHS_PREFIX/include/$(basename $header)"
	done
	install -Dm644 "lib/c-api/README.md" "$MCHS_PREFIX/include/wasmer-README.md"

	# make install-capi-lib
	pkgver="$(target/release/wasmer --version | cut -d\  -f2)"
	shortver="${pkgver%.*}"
	majorver="${shortver%.*}"
	install -Dm755 "target/$CARGO_TARGET_NAME/release/libwasmer_c_api.so" "$MCHS_PREFIX/lib/libwasmer.so.$pkgver"
	ln -sf "libwasmer.so.$pkgver" "$MCHS_PREFIX/lib/libwasmer.so.$shortver"
	ln -sf "libwasmer.so.$pkgver" "$MCHS_PREFIX/lib/libwasmer.so.$majorver"
	ln -sf "libwasmer.so.$pkgver" "$MCHS_PREFIX/lib/libwasmer.so"

	# make install-capi-staticlib
	install -Dm644 "target/$CARGO_TARGET_NAME/release/libwasmer_c_api.a" "$MCHS_PREFIX/lib/libwasmer.a"

	# make install-pkg-config
	target/release/wasmer config --pkg-config | install -Dm644 /dev/stdin "$MCHS_PREFIX/lib/pkgconfig/wasmer.pc"

	# make install-wapm (non-existant)
	install -Dm755 "wapm-cli/target/$CARGO_TARGET_NAME/release/wapm" "$MCHS_PREFIX/bin/wapm"

	cat <<- EOF > "$MCHS_PKG_TMPDIR/wasmer.sh"
	#!$MCHS_PREFIX/bin/sh
	export PATH=\$PATH:\$HOME/.wasmer/globals/wapm_packages/.bin
	EOF
	install -Dm644 "$MCHS_PKG_TMPDIR/wasmer.sh" "$MCHS_PREFIX/etc/profile.d/wasmer.sh"
