...
mkdir -p "${TMPDIR:-@MCHS_PREFIX@/tmp}"/.ldconfig
echo -e "#!@MCHS_PREFIX@/bin/bash\nexec true" > "${TMPDIR:-@MCHS_PREFIX@/tmp}"/.ldconfig/ldconfig
chmod 700 "${TMPDIR:-@MCHS_PREFIX@/tmp}"/.ldconfig/ldconfig
export PATH="$PATH:${TMPDIR:-@MCHS_PREFIX@/tmp}/.ldconfig"
...
# Cleanup.
rm -rf "${TMPDIR:-@MCHS_PREFIX@/tmp}"/.ldconfig
