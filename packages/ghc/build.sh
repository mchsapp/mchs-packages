MCHS_PKG_HOMEPAGE=https://www.haskell.org/ghc/
MCHS_PKG_DESCRIPTION="The Glasgow Haskell Compilation system"
MCHS_PKG_LICENSE="BSD 2-Clause, BSD 3-Clause, LGPL-2.1"
MCHS_PKG_MAINTAINER="@mchs"
MCHS_PKG_VERSION=8.10.1
MCHS_PKG_REVISION=6
MCHS_PKG_SRCURL=http://downloads.haskell.org/~ghc/${MCHS_PKG_VERSION}/ghc-${MCHS_PKG_VERSION}-src.tar.xz
MCHS_PKG_SHA256=4e3b07f83a266b3198310f19f71e371ebce97c769b14f0d688f4cbf2a2a1edf5
MCHS_PKG_DEPENDS="binutils, clang, iconv, libffi, llvm, ncurses"
MCHS_PKG_BUILD_IN_SRC=true
MCHS_PKG_NO_STATICSPLIT=true
MCHS_PKG_EXTRA_CONFIGURE_ARGS=" --disable-ld-override --build=x86_64-unknown-linux --host=x86_64-unknown-linux"
...
mchs_step_pre_configure() {
	...
	_WRAPPER_BIN=$MCHS_PKG_BUILDDIR/_wrapper/bin
	mkdir -p $_WRAPPER_BIN
	_WRAPPER_AR=$_WRAPPER_BIN/$MCHS_HOST_PLATFORM-ar
	cat > $_WRAPPER_AR <<-EOF
		#!$(command -v sh)
		exec $(command -v $AR) "\$@"
	EOF
	...
	MCHS_PKG_EXTRA_CONFIGURE_ARGS+=" --target=${MCHS_HOST_PLATFORM}"
	...
	# $MCHS_PREFIX/lib automatically gets added to the rpath that's generated
	if [ "$DYNAMIC_GHC_PROGRAMS" != "YES" ]; then
		echo "GhcStage2HcOpts += -optl-Wl,-rpath=$MCHS_PREFIX/lib" >> mk/build.mk
	fi
...
# host's libs. But of course, we need to link to McHs's libffi etc. and use the
...
+ifneq "\$3" "0"
+\$1_\$2_CONFIGURE_OPTS += --configure-option=--host=$MCHS_HOST_PLATFORM --configure-option=CXX=$CXX --configure-option=LD=$LD --configure-option=AR=$AR
...
+\$1_\$2_CONFIGURE_OPTS += --configure-option=--with-ffi-includes=$MCHS_PREFIX/include --configure-option=--with-ffi-libraries=$MCHS_PREFIX/lib
+\$1_\$2_CONFIGURE_OPTS += --configure-option=--with-iconv-includes=$MCHS_PREFIX/include --configure-option=--with-iconv-libraries=$MCHS_PREFIX/lib
+\$1_\$2_CONFIGURE_OPTS += --configure-option=--with-curses-includes=$MCHS_PREFIX/include/ncursesw --configure-option=--with-curses-libraries=$MCHS_PREFIX/lib
+endif
+
 ifeq "\$3" "0"
...
mchs_step_post_make_install() {
	# We may build GHC with `llc-9` etc., but only `llc` is present in McHs
	sed -i 's/"LLVM llc command", "llc.*"/"LLVM llc command", "llc"/' "$MCHS_PREFIX/lib/ghc-$MCHS_PKG_VERSION/settings"
	sed -i 's/"LLVM opt command", "opt.*"/"LLVM opt command", "opt"/' "$MCHS_PREFIX/lib/ghc-$MCHS_PKG_VERSION/settings"

	if [ "$DYNAMIC_GHC_PROGRAMS" = "YES" ]; then
		# Hack to turn the binaries' rpath from something relative to the build environment
		# into something that matches McHs's FHS. patchelf needs to be installed on the host.
		while IFS= read -r -d '' bin; do
			...
			if [ -n "$curr_rpath" ]; then
				...
				for i in "${!paths[@]}"; do
					...
					if [[ ${paths[$i]} != *ghc-$MCHS_PKG_VERSION* ]]; then
					...
					if [ "$fixed_path" != "${paths[$i]}" ]; then
						...
						fixed_path=$(echo "$MCHS_PREFIX/lib/ghc-$MCHS_PKG_VERSION/$fixed_path-"[[:digit:]]*)
						...
						if [ "$fixed_path" != "${paths[$i]}" ]; then
							new_rpath+=("$MCHS_PREFIX/lib/ghc-$MCHS_PKG_VERSION/${fixed_path##*/}")
						else
							new_rpath+=("$fixed_path")
						fi
					fi
				done
				# Make sure the standard McHs library path is the first entry
				[[ ! " ${paths_to_prepend[@]} " =~ " $MCHS_PREFIX/lib " ]] && paths_to_prepend=("$MCHS_PREFIX/lib" "${paths_to_prepend[@]}")
				# This isn't in the original rpath at all, but is needed
				paths_to_prepend+=("$MCHS_PREFIX/lib/ghc-$MCHS_PKG_VERSION/ghc-$MCHS_PKG_VERSION")
				local OIFS="$IFS"
				local IFS=':'
		...
		done < <(find "$MCHS_PREFIX/lib/ghc-$MCHS_PKG_VERSION/bin/" -type f -print0)
	fi
}

mchs_step_install_license() {
	install -Dm600 -t "$MCHS_PREFIX/share/doc/ghc" "$MCHS_PKG_SRCDIR/LICENSE"
}
