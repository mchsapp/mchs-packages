MCHS_PKG_HOMEPAGE=https://www.gnu.org/software/bash/
MCHS_PKG_DESCRIPTION="A sh-compatible shell that incorporates useful features from the Korn shell (ksh) and C shell (csh)"
MCHS_PKG_LICENSE="GPL-3.0"
MCHS_PKG_MAINTAINER="@mchs"
_MAIN_VERSION=5.1
_PATCH_VERSION=16
MCHS_PKG_VERSION=${_MAIN_VERSION}.${_PATCH_VERSION}
MCHS_PKG_SRCURL=https://mirrors.kernel.org/gnu/bash/bash-${_MAIN_VERSION}.tar.gz
MCHS_PKG_SHA256=cc012bc860406dcf42f64431bcd3d2fa7560c02915a601aba9cd597a39329baa
MCHS_PKG_DEPENDS="libandroid-support, libiconv, ncurses, readline (>= 8.0), mchs-tools"
MCHS_PKG_RECOMMENDS="command-not-found"
MCHS_PKG_BREAKS="bash-dev"
MCHS_PKG_REPLACES="bash-dev"
MCHS_PKG_ESSENTIAL=true
MCHS_PKG_BUILD_IN_SRC=true

MCHS_PKG_EXTRA_CONFIGURE_ARGS="--enable-multibyte --without-bash-malloc --with-installed-readline"
MCHS_PKG_EXTRA_CONFIGURE_ARGS+=" bash_cv_job_control_missing=present"
MCHS_PKG_EXTRA_CONFIGURE_ARGS+=" bash_cv_sys_siglist=yes"
MCHS_PKG_EXTRA_CONFIGURE_ARGS+=" bash_cv_func_sigsetjmp=present"
MCHS_PKG_EXTRA_CONFIGURE_ARGS+=" bash_cv_unusable_rtsigs=no"
MCHS_PKG_EXTRA_CONFIGURE_ARGS+=" ac_cv_func_mbsnrtowcs=no"
...
MCHS_PKG_EXTRA_CONFIGURE_ARGS+=" bash_cv_dev_fd=whacky"
...
# - https://github.com/mchs/mchs-app/issues/200
MCHS_PKG_EXTRA_CONFIGURE_ARGS+=" bash_cv_getcwd_malloc=yes"

MCHS_PKG_CONFFILES="etc/bash.bashrc etc/profile"

MCHS_PKG_RM_AFTER_INSTALL="share/man/man1/bashbug.1 bin/bashbug"
...
mchs_step_pre_configure() {
	...
	for PATCH_NUM in $(seq -f '%03g' ${_PATCH_VERSION}); do
		PATCHFILE=$MCHS_PKG_CACHEDIR/bash_patch_${PATCH_NUM}.patch
		mchs_download \
...
mchs_step_post_make_install() {
	sed -e "s|@MCHS_PREFIX@|$MCHS_PREFIX|" \
		-e "s|@MCHS_HOME@|$MCHS_ANDROID_HOME|" \
		$MCHS_PKG_BUILDER_DIR/etc-profile > $MCHS_PREFIX/etc/profile

	# /etc/bash.bashrc - System-wide .bashrc file for interactive shells. (config-top.h in bash source, patched to enable):
	sed -e "s|@MCHS_PREFIX@|$MCHS_PREFIX|" \
		-e "s|@MCHS_HOME@|$MCHS_ANDROID_HOME|" \
		$MCHS_PKG_BUILDER_DIR/etc-bash.bashrc > $MCHS_PREFIX/etc/bash.bashrc
}
