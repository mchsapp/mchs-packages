mchs_setup_gn() {
	...
	local GN_TARFILE=$MCHS_COMMON_CACHEDIR/gn_$GN_COMMIT.tar.gz
	local GN_SOURCE=https://gn.googlesource.com/gn/+archive/$GN_COMMIT.tar.gz

	if [ "${MCHS_PACKAGES_OFFLINE-false}" = "true" ]; then
		GN_FOLDER=$MCHS_SCRIPTDIR/build-tools/gn-$GN_COMMIT
	else
		GN_FOLDER=$MCHS_COMMON_CACHEDIR/gn-$GN_COMMIT
	fi
	...
	if [ "$MCHS_ON_DEVICE_BUILD" = "false" ]; then
		...
		if [[ "$MCHS_MAIN_PACKAGE_FORMAT" = "debian" && "$(dpkg-query -W -f '${db:Status-Status}\n' gn 2>/dev/null)" != "installed" ]] ||
                   [[ "$MCHS_MAIN_PACKAGE_FORMAT" = "pacman" && ! "$(pacman -Q gn 2>/dev/null)" ]]; then
