mchs_step_configure_cmake() {
	...
	BUILD_TYPE=Release
	test "$MCHS_DEBUG_BUILD" == "true" && BUILD_TYPE=Debug
	CMAKE_PROC=$MCHS_ARCH
	...
	if [ "$MCHS_ON_DEVICE_BUILD" = "false" ]; then
		...
		LDFLAGS+=" --target=$CCMCHS_HOST_PLATFORM"

		CMAKE_ADDITIONAL_ARGS+=("-DCMAKE_CROSSCOMPILING=True")
		CMAKE_ADDITIONAL_ARGS+=("-DCMAKE_LINKER=$MCHS_STANDALONE_TOOLCHAIN/bin/$LD $LDFLAGS")
		CMAKE_ADDITIONAL_ARGS+=("-DCMAKE_SYSTEM_NAME=Android")
		CMAKE_ADDITIONAL_ARGS+=("-DCMAKE_SYSTEM_VERSION=$MCHS_PKG_API_LEVEL")
		CMAKE_ADDITIONAL_ARGS+=("-DCMAKE_SYSTEM_PROCESSOR=$CMAKE_PROC")
		CMAKE_ADDITIONAL_ARGS+=("-DCMAKE_ANDROID_STANDALONE_TOOLCHAIN=$MCHS_STANDALONE_TOOLCHAIN")
	else
		CMAKE_ADDITIONAL_ARGS+=("-DCMAKE_LINKER=$(command -v $LD) $LDFLAGS")
	fi

	# XXX: CMAKE_{AR,RANLIB} needed for at least jsoncpp build to not
	# pick up cross compiled binutils tool in $MCHS_PREFIX/bin:
	cmake -G "$MCHS_CMAKE_BUILD" "$MCHS_PKG_SRCDIR" \
		...
		-DCMAKE_FIND_ROOT_PATH=$MCHS_PREFIX \
		-DCMAKE_FIND_ROOT_PATH_MODE_PROGRAM=NEVER \
		-DCMAKE_FIND_ROOT_PATH_MODE_INCLUDE=ONLY \
		-DCMAKE_FIND_ROOT_PATH_MODE_LIBRARY=ONLY \
		-DCMAKE_INSTALL_PREFIX=$MCHS_PREFIX \
		-DCMAKE_INSTALL_LIBDIR=$MCHS_PREFIX/lib \
