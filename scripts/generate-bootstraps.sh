...
MCHS_ARCHITECTURES=("aarch64" "arm" "i686" "x86_64")

# Can be changed by using '--repository' option.
REPO_BASE_URL="https://tuberboy.com/McHs/apt"
	...
	if [ ! -e "$package_tmpdir/package.deb" ]; then
		...
		(cd "$package_tmpdir"
			...
			if ! ${BOOTSTRAP_ANDROID10_COMPATIBLE}; then
				...
				tar tf "$data_archive" | sed -E -e 's@^\./@/@' -e 's@^/$@/.@' -e 's@^([^./])@/\1@' > "${BOOTSTRAP_ROOTFS}/${MCHS_PREFIX}/var/lib/dpkg/info/${package_name}.list"

				# Generate checksums (md5).
				tar xf "$data_archive"
				find data -type f -print0 | xargs -0 -r md5sum | sed 's@^\.$@@g' > "${BOOTSTRAP_ROOTFS}/${MCHS_PREFIX}/var/lib/dpkg/info/${package_name}.md5sums"
				...
				for file in conffiles postinst postrm preinst prerm; do
					if [ -f "${PWD}/${file}" ]; then
						cp "$file" "${BOOTSTRAP_ROOTFS}/${MCHS_PREFIX}/var/lib/dpkg/info/${package_name}.${file}"
					fi
				done
			fi
		)
...
show_usage() {
	...
	echo
	echo "Generate bootstrap archives for McHs application."
	echo
	echo "Options:"
	echo
	...
	echo
	echo "Architectures: ${MCHS_ARCHITECTURES[*]}"
	echo "Repository URL: ${REPO_BASE_URL}"
	echo "Prefix: ${MCHS_PREFIX}"
	echo
}
...
for package_arch in "${MCHS_ARCHITECTURES[@]}"; do
	...
	pull_package sed
	pull_package tar
	pull_package mchs-exec
	pull_package mchs-keyring
	pull_package mchs-tools
