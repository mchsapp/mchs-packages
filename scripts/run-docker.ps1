# PowerShell script to build McHs packages with Docker.
#
# Usage example:
#
# .\scripts\run-docker.ps1 ./build-package.sh -a arm libandroid-support

Set-Variable -Name IMAGE_NAME -Value "mchsapp/package-builder"
Set-Variable -Name CONTAINER_NAME -Value "mchs-package-builder"
...
if (-Not $?) {
    ...
    docker run `
        --detach `
        --name $CONTAINER_NAME `
        --volume "${PWD}:/home/builder/mchs-packages" `
        --tty `
        "$IMAGE_NAME"
}
