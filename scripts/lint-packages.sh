...
lint_package() {
	...
	(set +e +u
		...
		echo -n "MCHS_PKG_HOMEPAGE: "
		if [ -n "$MCHS_PKG_HOMEPAGE" ]; then
			if ! grep -qP '^https://.+' <<< "$MCHS_PKG_HOMEPAGE"; then
		...
		echo -n "MCHS_PKG_DESCRIPTION: "
		if [ -n "$MCHS_PKG_DESCRIPTION" ]; then
			str_length=$(($(wc -c <<< "$MCHS_PKG_DESCRIPTION") - 1))
		...
		echo -n "MCHS_PKG_LICENSE: "
		if [ -n "$MCHS_PKG_LICENSE" ]; then
			if [ "$MCHS_PKG_LICENSE" = "custom" ]; then
				echo "CUSTOM"
			elif [ "$MCHS_PKG_LICENSE" = "non-free" ]; then
				echo "NON-FREE"
			else
				if check_package_license "$MCHS_PKG_LICENSE"; then
					echo "PASS"
				else
					echo "INVALID"
					pkg_lint_error=true
		...
		echo -n "MCHS_PKG_MAINTAINER: "
		if [ -n "$MCHS_PKG_MAINTAINER" ]; then
			echo "PASS"
		else
			echo "NOT SET"
			pkg_lint_error=true
		...
		if [ -n "$MCHS_PKG_API_LEVEL" ]; then
			echo -n "MCHS_PKG_API_LEVEL: "

			if grep -qP '^[1-9][0-9]$' <<< "$MCHS_PKG_API_LEVEL"; then
				if [ $MCHS_PKG_API_LEVEL -lt 24 ] || [ $MCHS_PKG_API_LEVEL -gt 28 ]; then
		...
		echo -n "MCHS_PKG_VERSION: "
		if [ -n "$MCHS_PKG_VERSION" ]; then
			if grep -qiP '^([0-9]+\:)?[0-9][0-9a-z+\-\.]*$' <<< "$MCHS_PKG_VERSION"; then
				echo "PASS"
			else
		...
		if [ -n "$MCHS_PKG_REVISION" ]; then
			echo -n "MCHS_PKG_REVISION: "

			if grep -qP '^[1-9](\d{1,8})?$' <<< "$MCHS_PKG_REVISION"; then
				echo "PASS"
			else
		...
		if [ -n "$MCHS_PKG_SKIP_SRC_EXTRACT" ]; then
			echo -n "MCHS_PKG_SKIP_SRC_EXTRACT: "

			if [ "$MCHS_PKG_SKIP_SRC_EXTRACT" = "true" ] || [ "$MCHS_PKG_SKIP_SRC_EXTRACT" = "false" ]; then
				echo "PASS"
			else
		...
		if [ -n "$MCHS_PKG_SRCURL" ]; then
			echo -n "MCHS_PKG_SRCURL: "

			urls_ok=true
			for url in "${MCHS_PKG_SRCURL[@]}"; do
				if [ -n "$url" ]; then
			...
			echo -n "MCHS_PKG_SHA256: "
			if [ -n "$MCHS_PKG_SHA256" ]; then
				if [ "${#MCHS_PKG_SRCURL[@]}" -eq "${#MCHS_PKG_SHA256[@]}" ]; then
					sha256_ok=true

					for sha256 in "${MCHS_PKG_SHA256[@]}"; do
					...
					echo "LENGTHS OF 'MCHS_PKG_SRCURL' AND 'MCHS_PKG_SHA256' ARE NOT EQUAL"
					pkg_lint_error=true
				fi
			elif [ "${MCHS_PKG_SRCURL: -4}" == ".git" ]; then
				echo "NOT SET (acceptable since MCHS_PKG_SRCURL is git repo)"
			else
				echo "NOT SET"
				pkg_lint_error=true
			fi
		else
			if [ "$MCHS_PKG_SKIP_SRC_EXTRACT" != "true" ] && ! declare -F mchs_step_extract_package > /dev/null 2>&1; then
				echo "MCHS_PKG_SRCURL: NOT SET (set MCHS_PKG_SKIP_SRC_EXTRACT to 'true' if no sources downloaded)"
				pkg_lint_error=true
			fi
		fi

		if [ -n "$MCHS_PKG_METAPACKAGE" ]; then
			echo -n "MCHS_PKG_METAPACKAGE: "

			if [ "$MCHS_PKG_METAPACKAGE" = "true" ] || [ "$MCHS_PKG_METAPACKAGE" = "false" ]; then
				echo "PASS"
			else
				echo "INVALID (allowed: true / false)"
				pkg_lint_error=true
		...
		if [ -n "$MCHS_PKG_ESSENTIAL" ]; then
			echo -n "MCHS_PKG_ESSENTIAL: "
			if [ "$MCHS_PKG_ESSENTIAL" = "true" ] || [ "$MCHS_PKG_ESSENTIAL" = "false" ]; then
				echo "PASS"
			else
				echo "INVALID (allowed: true / false)"
				pkg_lint_error=true
		...
		if [ -n "$MCHS_PKG_NO_STATICSPLIT" ]; then
			echo -n "MCHS_PKG_NO_STATICSPLIT: "

			if [ "$MCHS_PKG_NO_STATICSPLIT" = "true" ] || [ "$MCHS_PKG_NO_STATICSPLIT" = "false" ]; then
				echo "PASS"
			else
				echo "INVALID (allowed: true / false)"
				pkg_lint_error=true
		...
		if [ -n "$MCHS_PKG_BUILD_IN_SRC" ]; then
			echo -n "MCHS_PKG_BUILD_IN_SRC: "

			if [ "$MCHS_PKG_BUILD_IN_SRC" = "true" ] || [ "$MCHS_PKG_BUILD_IN_SRC" = "false" ]; then
				echo "PASS"
			else
				echo "INVALID (allowed: true / false)"
				pkg_lint_error=true
		...
		if [ -n "$MCHS_PKG_HAS_DEBUG" ]; then
			echo -n "MCHS_PKG_HAS_DEBUG: "

			if [ "$MCHS_PKG_HAS_DEBUG" = "true" ] || [ "$MCHS_PKG_HAS_DEBUG" = "false" ]; then
				echo "PASS"
			else
				echo "INVALID (allowed: true / false)"
				pkg_lint_error=true
		...
		if [ -n "$MCHS_PKG_PLATFORM_INDEPENDENT" ]; then
			echo -n "MCHS_PKG_PLATFORM_INDEPENDENT: "

			if [ "$MCHS_PKG_PLATFORM_INDEPENDENT" = "true" ] || [ "$MCHS_PKG_PLATFORM_INDEPENDENT" = "false" ]; then
				echo "PASS"
			else
				echo "INVALID (allowed: true / false)"
				pkg_lint_error=true
		...
		if [ -n "$MCHS_PKG_HOSTBUILD" ]; then
			echo -n "MCHS_PKG_HOSTBUILD: "

			if [ "$MCHS_PKG_HOSTBUILD" = "true" ] || [ "$MCHS_PKG_HOSTBUILD" = "false" ]; then
				echo "PASS"
			else
				echo "INVALID (allowed: true / false)"
				pkg_lint_error=true
		...
		if [ -n "$MCHS_PKG_FORCE_CMAKE" ]; then
			echo -n "MCHS_PKG_FORCE_CMAKE: "

			if [ "$MCHS_PKG_FORCE_CMAKE" = "true" ] || [ "$MCHS_PKG_FORCE_CMAKE" = "false" ]; then
				echo "PASS"
			else
				echo "INVALID (allowed: true / false)"
				pkg_lint_error=true
		...
		if [ -n "$MCHS_PKG_RM_AFTER_INSTALL" ]; then
			echo -n "MCHS_PKG_RM_AFTER_INSTALL: "
			file_path_ok=true
			...
			done <<< "$MCHS_PKG_RM_AFTER_INSTALL"
			unset file_path
		...
		if [ -n "$MCHS_PKG_SERVICE_SCRIPT" ]; then
			echo -n "MCHS_PKG_SERVICE_SCRIPT: "
			array_length=${#MCHS_PKG_SERVICE_SCRIPT[@]}
			if [ $(( $array_length & 1 )) -eq 1 ]; then
				echo "INVALID (MCHS_PKG_SERVICE_SCRIPT has to be an array of even length)"
				pkg_lint_error=true
			else
				echo "PASS"
			fi
