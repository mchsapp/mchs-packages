...
ANDROID_NDK_FILE=android-ndk-r${MCHS_NDK_VERSION}-linux.zip
...
if [ ! -d $NDK ]; then
	...
	rm -Rf android-ndk-r$MCHS_NDK_VERSION
	unzip -q ndk.zip
	mv android-ndk-r$MCHS_NDK_VERSION $(basename $NDK)
	rm ndk.zip
fi
...
yes | $ANDROID_HOME/cmdline-tools/bin/sdkmanager --sdk_root=$ANDROID_HOME \
		"platform-tools" \
		"build-tools;${MCHS_ANDROID_BUILD_TOOLS_VERSION}" \
