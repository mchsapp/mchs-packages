...
def parse_build_file_dependencies(path):
    ...
    with open(path, encoding="utf-8") as build_script:
        for line in build_script:
            if line.startswith( ('MCHS_PKG_DEPENDS', 'MCHS_PKG_BUILD_DEPENDS', 'MCHS_SUBPKG_DEPENDS', 'MCHS_PKG_DEVPACKAGE_DEPENDS') ):
...
class McHsPackage(object):
    ...
    def __init__(self, dir_path, fast_build_mode):
        ...
        if os.getenv('MCHS_ON_DEVICE_BUILD') == "true":
            always_deps = ['libc++']
        ...
        for filename in os.listdir(self.dir):
            if not filename.endswith('.subpackage.sh'):
                continue
            subpkg = McHsSubPackage(self.dir + '/' + filename, self)

            self.subpkgs.append(subpkg)
            self.deps.add(subpkg.name)
            self.deps |= subpkg.deps

        subpkg = McHsSubPackage(self.dir + '/' + self.name + '-static' + '.subpackage.sh', self, virtual=True)
        self.subpkgs.append(subpkg)
...
def read_packages_from_directories(directories, fast_build_mode):
    """Construct a map from package name to McHsPackage.
    ...
    for package_dir in directories:
        for pkgdir_name in sorted(os.listdir(package_dir)):
            ...
            if os.path.isfile(dir_path + '/build.sh'):
                new_package = McHsPackage(package_dir + '/' + pkgdir_name, fast_build_mode)
    ...
    for pkg in all_packages:
        for dependency_name in pkg.deps:
            ...
            if fast_build_mode or not isinstance(pkg, McHsSubPackage):
...
def generate_full_buildorder(pkgs_map):
    ...
    # List of all McHsPackages without dependencies
