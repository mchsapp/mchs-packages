...
: ${MCHS_BUILDER_IMAGE_NAME:=mchsapp/package-builder}
: ${CONTAINER_NAME:=mchs-package-builder}

USER=builder

if [ -n "${MCHS_DOCKER_USE_SUDO-}" ]; then
	SUDO="sudo"
else
	SUDO=""
fi

echo "Running container '$CONTAINER_NAME' from image '$MCHS_BUILDER_IMAGE_NAME'..."
...
$SUDO docker start $CONTAINER_NAME >/dev/null 2>&1 || {
	...
	$SUDO docker run \
		--detach \
		--name $CONTAINER_NAME \
		--volume $REPOROOT:$CONTAINER_HOME_DIR/mchs-packages \
		--tty \
		$MCHS_BUILDER_IMAGE_NAME
	if [ "$UNAME" != Darwin ]; then
