...
# Description:   A script to build bootstrap archives for the mchs-app
#                from local package sources instead of debs published in
#                apt repo like done by generate-bootstrap.sh. It allows
#                bootstrap archives to be easily built for (forked) mchs
...
# supported by McHs application.
# Override with option '--architectures'.
MCHS_DEFAULT_ARCHITECTURES=("aarch64" "arm" "i686" "x86_64")
MCHS_ARCHITECTURES=("${MCHS_DEFAULT_ARCHITECTURES[@]}")

MCHS_PACKAGES_DIRECTORY="/home/builder/mchs-packages"
MCHS_BUILT_DEBS_DIRECTORY="$MCHS_PACKAGES_DIRECTORY/output"
...
build_package() {
	...
	cd "$MCHS_PACKAGES_DIRECTORY"
	echo $'\n\n\n'"[*] Building '$package_name'..."
	exec 99>&1
	build_output="$("$MCHS_PACKAGES_DIRECTORY"/build-package.sh "${BUILD_PACKAGE_OPTIONS[@]}" -a "$package_arch" "$package_name" 2>&1 | tee >(cat - >&99); exit ${PIPESTATUS[0]})";
	return_value=$?
...
extract_debs() {
	...
	for deb in *.deb; do
		...
		echo "[*] Extracting '$deb'..."
		(cd "$package_tmpdir"
			ar x "$MCHS_BUILT_DEBS_DIRECTORY/$deb"
			...
			if ! ${BOOTSTRAP_ANDROID10_COMPATIBLE}; then
				...
				tar tf "$data_archive" | sed -E -e 's@^\./@/@' -e 's@^/$@/.@' -e 's@^([^./])@/\1@' > "${BOOTSTRAP_ROOTFS}/${MCHS_PREFIX}/var/lib/dpkg/info/${current_package_name}.list"

				# Generate checksums (md5).
				tar xf "$data_archive"
				find data -type f -print0 | xargs -0 -r md5sum | sed 's@^\.$@@g' > "${BOOTSTRAP_ROOTFS}/${MCHS_PREFIX}/var/lib/dpkg/info/${current_package_name}.md5sums"
				...
				tar xf "$control_archive"
				{
					...
					echo
				} >> "${BOOTSTRAP_ROOTFS}/${MCHS_PREFIX}/var/lib/dpkg/status"

				# Additional data: conffiles & scripts
				for file in conffiles postinst postrm preinst prerm; do
					if [ -f "${PWD}/${file}" ]; then
						cp "$file" "${BOOTSTRAP_ROOTFS}/${MCHS_PREFIX}/var/lib/dpkg/info/${current_package_name}.${file}"
					fi
				done
			fi
		)
...
create_bootstrap_archive() {
	...
	(cd "${BOOTSTRAP_ROOTFS}/${MCHS_PREFIX}"
	...
	mv -f "${BOOTSTRAP_TMPDIR}/bootstrap-${1}.zip" "$MCHS_PACKAGES_DIRECTORY/"
...
build_bootstrap_trap() {
	...
	[ -h "$MCHS_BUILT_PACKAGES_DIRECTORY" ] && rm -f "$MCHS_BUILT_PACKAGES_DIRECTORY"
...
mchs-app from local package sources instead of debs published in
apt repo like done by generate-bootstrap.sh. It allows bootstrap archives
to be easily built for (forked) mchs apps without having to publish
an apt repo first.
...
MCHS_APP_PACKAGE in 'scrips/properties.sh'. It defaults to 'com.mchs'.
...
echo $'\n'"MCHS_APP_PACKAGE: \"$MCHS_APP_PACKAGE\""
echo "MCHS_PREFIX: \"${MCHS_PREFIX[*]}\""
echo "MCHS_ARCHITECTURES: \"${MCHS_ARCHITECTURES[*]}\""
...
main() {
	...
	for package_arch in "${MCHS_ARCHITECTURES[@]}"; do
		if [[ " ${MCHS_DEFAULT_ARCHITECTURES[*]} " != *" $package_arch "* ]]; then
			echo "Unsupported architecture '$package_arch' for in architectures list: '${MCHS_ARCHITECTURES[*]}'" 1>&2
			echo "Supported architectures: '${MCHS_DEFAULT_ARCHITECTURES[*]}'" 1>&2
			return 1
		fi
	done
	...
	for package_arch in "${MCHS_ARCHITECTURES[@]}"; do
		...
		# The mchs_step_finish_build stores package version in .built-packages directory, but
		...
		# to rebuild packages
		MCHS_BUILT_PACKAGES_DIRECTORY_FOR_ARCH="$MCHS_BUILT_PACKAGES_DIRECTORY-$package_arch"
		mkdir -p "$MCHS_BUILT_PACKAGES_DIRECTORY_FOR_ARCH"

		if [ -f "$MCHS_BUILT_PACKAGES_DIRECTORY" ] || [ -d "$MCHS_BUILT_PACKAGES_DIRECTORY" ]; then
			rm -rf "$MCHS_BUILT_PACKAGES_DIRECTORY"
		fi

		ln -sf "$MCHS_BUILT_PACKAGES_DIRECTORY_FOR_ARCH" "$MCHS_BUILT_PACKAGES_DIRECTORY"

		if [[ $FORCE_BUILD_PACKAGES == "1" ]]; then
			rm -f "$MCHS_BUILT_PACKAGES_DIRECTORY_FOR_ARCH"/*
			rm -f "$MCHS_BUILT_DEBS_DIRECTORY"/*
		fi
		...
		# Create initial directories for $MCHS_PREFIX
		if ! ${BOOTSTRAP_ANDROID10_COMPATIBLE}; then
			mkdir -p "${BOOTSTRAP_ROOTFS}/${MCHS_PREFIX}/etc/apt/apt.conf.d"
			mkdir -p "${BOOTSTRAP_ROOTFS}/${MCHS_PREFIX}/etc/apt/preferences.d"
			mkdir -p "${BOOTSTRAP_ROOTFS}/${MCHS_PREFIX}/var/lib/dpkg/info"
			mkdir -p "${BOOTSTRAP_ROOTFS}/${MCHS_PREFIX}/var/lib/dpkg/triggers"
			mkdir -p "${BOOTSTRAP_ROOTFS}/${MCHS_PREFIX}/var/lib/dpkg/updates"
			mkdir -p "${BOOTSTRAP_ROOTFS}/${MCHS_PREFIX}/var/log/apt"
			touch "${BOOTSTRAP_ROOTFS}/${MCHS_PREFIX}/var/lib/dpkg/available"
			touch "${BOOTSTRAP_ROOTFS}/${MCHS_PREFIX}/var/lib/dpkg/status"
		fi
		mkdir -p "${BOOTSTRAP_ROOTFS}/${MCHS_PREFIX}/tmp"
